...
`);
  ...
  if (transport === 'stdio') {
    ...
    if (!authToken) {
      throw new Error('Authentication token is required for HTTP transport. Please provide a token using the --auth-token argument or AUTH_TOKEN environment variable.')
    }
