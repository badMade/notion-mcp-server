...
export class OpenAPIToMCPConverter {
  ...
  private convertComponentsToJsonSchema(): Record<string, IJsonSchema> {
    // ⚡ Bolt Optimization: Cache the computed component schemas
    // This avoids recalculating the $defs for every parameter and response,
    // which previously caused O(N * M) behavior (N operations * M component schemas)
    // reducing large spec conversion times from seconds to milliseconds.
